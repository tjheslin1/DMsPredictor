...
abstract class SingleTargetAttackSpell extends Spell with LazyLogging {
  ...
  def additionalEffect[_: RS](target: Combatant, attackResult: AttackResult): Combatant = target
  ...
  def effect[_: RS](
    ...
    val damagedTarget = target
      .copy(creature = target.creature.updateHealth(dmg, damageType, attackResult))
