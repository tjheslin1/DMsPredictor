...
class VampireAbilitiesSpec extends UnitSpecBase {
    ...
    "target a creature who is grappled" in {
      forAll { (vampire: Vampire, fighter: Fighter, cleric: Cleric) =>
        new TestContext {
          ...
          val (_, List(_, Combatant(_, updatedCleric: Cleric))) = bite(1)(vampireCombatant)
    ...
    "restore the Vampires health equal to the necrotic damage dealt" in {
      forAll { (vampire: Vampire, cleric: Cleric) =>
        new TestContext {
          ...
          val (Combatant(_, updatedVampire: Vampire), _) = bite(1)(vampireCombatant)
    ...
    "reduce the creatures maxHealth (and health accordingly) equal to the necrotic damage taken" in {
      forAll { (vampire: Vampire, cleric: Cleric) =>
        new TestContext {
          ...
          val (_, List(Combatant(_, updatedCleric: Cleric))) = bite(1)(vampireCombatant)
            .useAbility(List(clericCombatant), LowestFirst)
    ...
    "attempt to grapple a target instead of dealing damage on its first attack" in {
      forAll { (vampire: Vampire, cleric: Cleric) =>
        new TestContext {
          ...
          val (_, List(Combatant(_, updatedCleric: Cleric))) = unarmedStrike(1)(vampireCombatant)
            .useAbility(List(clericCombatant), LowestFirst)
  ...
  "charm" should {
    ...
    "apply the Charmed condition if the saving throw failed" in {
      forAll { (vampire: Vampire, cleric: Cleric, fighter: Fighter) =>
        new TestContext {
          ...
          val (_, List(Combatant(_, updatedCleric: Cleric), _)) = charm(1)(vampireCombatant)
            .useAbility(List(clericCombatant, fighterCombatant), LowestFirst)
    ...
    "not apply the Charmed condition if the enemy has the CharmImmunity condition" in {
      forAll { (vampire: Vampire, cleric: Cleric) =>
        new TestContext {
          ...
          val vampireCombatant = vampire.withCombatIndex(1)
          val clericCombatant = cleric.withWisdom(2).withCondition(CharmImmunity).withCombatIndex(2)

          val (_, List(Combatant(_, updatedCleric: Cleric))) = charm(1)(vampireCombatant)
            .useAbility(List(clericCombatant), LowestFirst)
    ...
    "only target enemies who do not have immunity to the Charm condition" in new TestContext {
      implicit val roll: RollStrategy = _ => RollResult(15)

      val vampire = random[Vampire].withCombatIndex(1)

      val lowHpCharmImmuneCleric = random[Cleric]
        .withConditionImmunity(CharmedCondition)
        .withHealth(5)
        .withMaxHealth(50)
        .withCombatIndex(2)

      val highHpFighter = random[Fighter]
      ...
      val (_, List(Combatant(_, updatedCleric: Cleric), Combatant(_, updatedFighter: Fighter))) =
        charm(1)(vampire).useAbility(List(lowHpCharmImmuneCleric, highHpFighter), LowestFirst)

      updatedCleric.conditions should contain theSameElementsAs List.empty[Condition]

      updatedFighter.conditions should contain theSameElementsAs List(Charmed(Vampire.CharmDC))
    }
    ...
    "not be triggered if all enemies that are not charmed have the CharmImmunity condition" in {
      forAll { (vampire: Vampire, cleric: Cleric, fighter: Fighter) =>
        new TestContext {
          ...
          charm(1)(vampireCombatant).triggerMet(
            List(charmedCleric, charmImmuneFighter)) shouldBe false
        }
      }
    }

    "not be triggered if all enemies are immune to the Charm condition" in {
      forAll { (vampire: Vampire, cleric: Cleric, fighter: Fighter) =>
        new TestContext {
          implicit val roll: RollStrategy = _ => RollResult(15)
          ...
          val charmImmuneCleric = cleric.withConditionImmunity(CharmedCondition).withCombatIndex(2)
          val charmImmuneFighter = fighter
            .withConditionImmunity(CharmedCondition)
            .withCombatIndex(3)

          charm(1)(vampireCombatant).triggerMet(
            List(charmImmuneCleric, charmImmuneFighter)) shouldBe false
        }
      }
    }
  }
