    ...
    "attack a number of times equal to the total provided" in {
      ...
      forAll { (testMonster: TestMonster, fighter: Fighter) =>
        new TestContext {
          ...
          val (_, List(Combatant(_, updatedFighter: Fighter))) = multiAttack(
            Priority,
            totalNumberOfAttacks)(monsterCombatant)
    ...
    "delegate to a SingleAttack ability" in {
      ...
      forAll { (testMonster: TestMonster, fighter: Fighter) =>
        new TestContext {
          ...
          val singleAttackAbility: CombatantAbility = trackedAbility(
            3,
            action = SingleAttack,
          ...
          val monsterCombatant = testMonster
            ...
            .withAbilities(
              List(
                multiAttack(Priority, totalNumberOfAttacks),
                trackedAbility(2),
                singleAttackAbility))
            .withCombatIndex(1)
