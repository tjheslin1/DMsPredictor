...
class MultiTargetSavingThrowSpellSpec extends UnitSpecBase {
  ...
  "effect" should {
    "roll the damage once for all targets" in new TestContext {
      ...
      val monsterOne = random[TestMonster]
        .withHealth(10)
        .withSavingThrowScores(dexterity = -4)
        .withCombatIndex(2)
      val monsterTwo = random[TestMonster]
        .withHealth(10)
        .withSavingThrowScores(dexterity = -4)
        .withCombatIndex(3)
    ...
    "deal full damage if saving throw failed" in {
      forAll {
        (
            cleric: Cleric,
            testMonsterOne: TestMonster,
            testMonsterTwo: TestMonster,
            testMonsterThree: TestMonster
        ) =>
          new TestContext {
            implicit override val roll: RollStrategy = _ => RollResult(10)

            val savingThrowSpell = trackedMultiTargetSavingThrowSpell(
              1,
              Dexterity,
              damageOnSave = false)
            ...
            val (
              _,
              List(
                Combatant(_, updatedMonsterOne: TestMonster),
                Combatant(_, updatedMonsterTwo: TestMonster),
                Combatant(_, updatedMonsterThree: TestMonster))) = savingThrowSpell.effect(
              trackedCleric,
              savingThrowSpell.spellLevel,
              List(monsterOne, monsterTwo, monsterThree))
