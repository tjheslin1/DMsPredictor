...
class BasePaladinSpec extends UnitSpecBase {
  ...
  "paladinWeapon" should {
    "apply +2 to hit bonus for a one handed melee weapon with the Dueling fighting style" in new TestContext {
      ...
      val duelingPaladin = random[Paladin].withFightingStyle(Dueling)

      paladinWeapon(duelingPaladin, sword, none[Equipment]).hitBonus shouldBe 2
    }
    ...
    "not apply a hit bonus for a two handed melee weapon with the Dueling fighting style" in new TestContext {
      ...
      paladinWeapon(duelingPaladin, sword, none[Equipment]).hitBonus shouldBe 0
    }
    ...
    "not apply a hit bonus for a one handed melee weapon with the Dueling fighting style when wielding another weapon in off hand" in new TestContext {
      ...
      paladinWeapon(duelingPaladin, sword, sword.some).hitBonus shouldBe 0
    }
  ...
  "armourClass" should {
    "calculate default armour class for no armour and no shield" in new TestContext {
      armourClassWithFightingStyle(
        ...
        NoArmour,
        none[Equipment],
        List.empty[PaladinFightingStyle]) shouldBe 10
    }
    ...
    "calculate armour class for wearing armour but no shield" in new TestContext {
      armourClassWithFightingStyle(
        ...
        ChainMail,
        none[Equipment],
        List.empty[PaladinFightingStyle]) shouldBe 16
    }
    ...
    "calculate armour class for wearing a shield but no armour" in new TestContext {
      armourClassWithFightingStyle(
        ...
        NoArmour,
        Shield.some,
        List.empty[PaladinFightingStyle]) shouldBe 12
    }
    ...
    "calculate armour class for wearing armour and a shield" in new TestContext {
      armourClassWithFightingStyle(
        ...
        ChainMail,
        Shield.some,
        List.empty[PaladinFightingStyle]) shouldBe 18
    }

    "calculate armour class for wearing armour, shield and with high dexterity" in new TestContext {
      armourClassWithFightingStyle(
        BaseStats(10, 16, 10, 10, 10, 10),
        ChainShirt,
        Shield.some,
        List.empty[PaladinFightingStyle]) shouldBe 17
    }
