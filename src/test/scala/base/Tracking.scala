...
trait Tracking {
  ...
  val trackedOffHandSword = Weapon(
    "sword",
    Melee,
    Slashing,
    isTwoHanded = false,
  ...
  def trackedAbility(
      ...
      updatedTracking: => Unit = trackedAbilityUsed = true
  ...
  def otherTrackedAbility(
      order: Int,
      action: AbilityAction = WholeAction,
      `updatedTracking`: => Unit = otherTrackedAbilityUsed = true
  ): CombatantAbility = trackedAbility(
    order,
    action = `action`,
  ...
  def trackedSingleTargetSavingThrowSpell(
      ...
      higherSpellSlot: Boolean = true,
      additionalTargetEffect: (SpellCaster, Combatant, List[Combatant], Boolean) => (
          SpellCaster,
          Combatant,
          List[Combatant]) = (c, t, o, s) => (c, t, o)
  ): Spell =
    new SingleTargetSavingThrowSpell {
      ...
      ): (SpellCaster, Combatant, List[Combatant]) = additionalTargetEffect(
        spellCaster,
        target,
        others,
        savingThrowPassed)
  ...
  def trackedMultiTargetSavingThrowSpell(
      spellLvl: SpellLevel,
      savingAttribute: Attribute,
      damageOnSave: Boolean = false,
      higherSpellSlot: Boolean = true
  ...
  val compareHealth: (Combatant, Combatant) => Int =
    (x, y) =>
      if (x.creature.isConscious)
        1
      else if (x.creature.isConscious)
        1
      else
  ...
  def trackedConditionSpell(
      ...
      higherSpellSlot: Boolean = true,
      priority: (Combatant, Combatant) => Int = compareHealth
  ): ConditionSpell =
    new ConditionSpell {
      ...
      val conditionTargetsPriority: Ordering[Combatant] =
        (x: Combatant, y: Combatant) => priority(x, y)
      ...
      def conditionFrom(spellCaster: SpellCaster): Condition = Paralyzed(10, 10, attribute)
  ...
  def trackedMultiTargetBuffSpell(
      spellLvl: SpellLevel,
      condition: Condition,
      numTargets: Int = 3,
  ...
  def setHealthToOneEffect(
      spellCaster: SpellCaster,
      target: Combatant
  ): (SpellCaster, Combatant) = {
    val oneHpTarget =
      (Combatant.creatureLens composeLens Creature.creatureHealthLens).set(1)(target)
  ...
  def trackedInstantEffectSpell(
      spellLvl: SpellLevel,
      trackedEffect: (SpellCaster, Combatant) => (SpellCaster, Combatant) = setHealthToOneEffect,
      higherSpellSlot: Boolean = false
  ): SingleTargetInstantEffectSpell =
  ...
  def trackedStartOfTurnCondition(
      dc: Int,
  ...
  def trackedCondition(
      ...
      onRemoved: Creature => Creature = c => c
  ): Condition =
    new Condition {
